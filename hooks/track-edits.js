 ...
 */
/**
 * Read recent edits from the edit log within the last 30 minutes.
 * Shared utility also used by stop-reminders.js (both standalone hooks).
 */
function getRecentEdits(sessionId) {
  try {
    ...
    return lines
      ...
      .filter(e => e && new Date(e.timestamp) > cutoff && (sessionId ? e.sessionId === sessionId : e.sessionId === null));
  } catch {
    return [];
  }
}
